...
2)cat /etc/os-release has give me version specific details of the OS.

*CPU*
3)top-mostly there is no cpu usage 99.9% cpu is idle

*Memory*
4)free -h-give the memory details in humansize readable format as per the finding from 3.7gb total 3.2g is still available 

As per CPU and Memory there shall not be any issue to run nginx.

*disk_usage*
5)df -h -> as per this disk space is merely used only 2%.

*networking*

6)ss -tuln -Tcp port 80 is listening hence conclusion nginx service is active.
7)curl -i http://localhost-confirms by givinc below mentioned out that nginx is working fine.
HTTP/1.1 200 OK
