...
Used against multiple patterns instead of multiple if-else conditions.
  case "$fruit" in
...
Used to iterate or list down data from a list or perform arithmetic logic.
  List-based
...
### 2. `while` loop
A "true" loop; if the logic is true, it keeps iterating until stopped.
...
### 3. `until` loop
Opposite to while; a "false" loop that runs until the logic becomes true.
...
`break` stops the loop once triggered. `continue` skips the specific condition but finishes the loop.
  for i in {1..5}; do
...
Iterates over whatever file type is mentioned after the asterisk.
  for file in *.log; do
      echo $file
  done
...
Lets us read data inside a file one line at a time to run commands.
...
Groups logic in curly brackets. Call it simply by writing its name.
  new_function() {
      echo "hello yogesh"
  }
...
Functions use `$1`, `$2` internally. `return` is for success/error codes inside functions.
  new_function() {
      if [ "$1" = "hi" ]; then
...
Variables that can only be used inside the function they are defined in.
  new_function() {
...
### 1. `grep`
Search patterns with `-i` (case insensitive), `-c` (count), `-v` (invert), etc.
  grep -i "GREP" 1.log
  grep -c "grep" 1.log


### 2. `awk`
Useful for column separation using `-F`. Supports `BEGIN` and `END` patterns.
  awk -F ":" 'BEGIN {print "START"} {print $1} END {print "End"}' names.txt


### 3. `sed`
Find and replace tool that works line-by-line. Use `-i` for in-place edits.
  sed 's/Pune/Mumbai/g' names.txt
  sed -i 's/old/new/g' names.txt


### 4. `cut`
Extracts specific columns using a delimiter (`-d`) and field (`-f`).
  cut -d ":" -f 2-4 names.txt


### 5. `sort`
Sorts data alphabetically (`-d`), numerically (`-n`), or reverse (`-r`). `-u` gives unique lines.
  sort -r names.txt # Reverse alphabetical (Z-A)
  sort -u fruits.txt # Unique items only


### 6. `tr` (Translate)
Used to translate or delete characters. It maps characters in set1 to set2.
**Your Mapping Example:** `tr 'yogesh' 'sourav'`
*   `y` → `s`, `o` → `o`, `g` → `u`, `e` → `r`, `s` → `a`, `h` → `v`
  cat names.txt | tr 'yogesh' 'sourav'
  cat names.txt | tr -d ':' # Deletes all colons


### 7. `wc` (Word Count)
Counts lines (`-l`), words (`-w`), and characters.
  wc -l names.txt
...
## Task 6: Patterns and One-Liners
